...
class BatchREINFORCE:
    def __init__(self, env, policy, baseline,
        ...
        self.clip = 0.2             # epsilon for restricting the range kl-divergence
    ...
    def ppo_pg(self, observations, actions, advantages):
        adv_var = Variable(torch.from_numpy(advantages).float(), requires_grad=False)
        old_dist_info = self.policy.old_dist_info(observations, actions)
        new_dist_info = self.policy.new_dist_info(observations, actions)
        LR = self.policy.likelihood_ratio(new_dist_info, old_dist_info)
        surr1 = LR*adv_var
        surr2 = torch.clamp(LR, 1 - self.clip, 1 + self.clip) * adv_var
        cpi_surr =  (torch.min(surr1, surr2)).mean()
        kl_grad = torch.autograd.grad(cpi_surr, self.policy.trainable_params)
        kl_grad = np.concatenate([g.contiguous().view(-1).data.numpy() for g in kl_grad])
        return kl_grad
